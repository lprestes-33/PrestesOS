...
def test_notebooklm_service_resolves_auth_from_env(prestes_base_dir, database_service, log_service):
    config = ConfigService(base_dir=prestes_base_dir)
    bus = EventBus(db_service=database_service, log_service=log_service)
    service = NotebookLMService(
        config_service=config,
        event_bus=bus,
        environment={"NOTEBOOKLM_ACCESS_TOKEN": "env-token"},
    )
...
def test_notebooklm_service_resolves_auth_from_file(prestes_base_dir, database_service, log_service):
    credentials_path = prestes_base_dir / "config" / "notebooklm_credentials.json"
    credentials_path.parent.mkdir(parents=True, exist_ok=True)
    credentials_path.write_text('{"access_token":"file-token"}', encoding="utf-8")

    config = ConfigService(base_dir=prestes_base_dir)
    bus = EventBus(db_service=database_service, log_service=log_service)
    service = NotebookLMService(config_service=config, event_bus=bus, environment={})
...
def test_notebooklm_service_marks_expired_file_token(prestes_base_dir, database_service, log_service):
    credentials_path = prestes_base_dir / "config" / "notebooklm_credentials.json"
    credentials_path.parent.mkdir(parents=True, exist_ok=True)
...
def test_notebooklm_service_returns_status(prestes_base_dir, database_service, log_service):
    config = ConfigService(base_dir=prestes_base_dir)
    bus = EventBus(db_service=database_service, log_service=log_service)
    service = NotebookLMService(
        config_service=config,
    ...
    assert status.provider == "notebooklm-api-preparado"
    assert status.auth.source == "env"
    assert status.default_notebook == "PrestesOS"
