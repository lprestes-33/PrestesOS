...
class NotebookLMAuthState:
    """Responsabilidade: representar o estado efetivo da autenticacao NotebookLM."""
...
class NotebookLMStatus:
    """Responsabilidade: representar o status local da integracao NotebookLM."""
...
class NotebookLMService:
    """Responsabilidade: preparar a integracao futura do PrestesOS com NotebookLM."""
    ...
    def _credentials_path(self) -> Path:
        return Path(self.config.get("notebooklm.credentials_path")).expanduser()
    ...
    def _token_env_name(self) -> str:
        env_name = self.config.get("notebooklm.access_token_env", "NOTEBOOKLM_ACCESS_TOKEN")
        return str(env_name).strip() or "NOTEBOOKLM_ACCESS_TOKEN"

    def _credentials_access_token_key(self) -> str:
        key_name = self.config.get("notebooklm.credentials_access_token_key", "access_token")
        return str(key_name).strip() or "access_token"

    def _credentials_expires_at_key(self) -> str:
        key_name = self.config.get("notebooklm.credentials_expires_at_key", "expires_at")
    ...
    def resolve_auth(self) -> NotebookLMAuthState:
        ...
        if env_token:
            return NotebookLMAuthState(
                ...
                expires_at=None,
                is_expired=False,
                message=f"Token NotebookLM carregado da variavel de ambiente {env_name}.",
            )
        ...
        if isinstance(access_token, str) and access_token.strip() and not is_expired:
            return NotebookLMAuthState(
                ...
                is_expired=False,
                message=f"Token NotebookLM carregado do arquivo {credentials_path}.",
            )
        ...
        if isinstance(access_token, str) and access_token.strip() and is_expired:
            return NotebookLMAuthState(
                ...
                is_expired=True,
                message=f"Token NotebookLM do arquivo {credentials_path} esta expirado.",
            )
        ...
        return NotebookLMAuthState(
            ...
            expires_at=None,
            is_expired=False,
            message="Nenhum token NotebookLM valido foi encontrado.",
        )

    def status(self) -> NotebookLMStatus:
        auth = self.resolve_auth()
        status = NotebookLMStatus(
            provider=self.config.get("notebooklm.provider", "notebooklm-api-preparado"),
            auth=auth,
            default_notebook=self.config.get("notebooklm.default_notebook", "PrestesOS"),
            max_sources=self.config.get("notebooklm.max_sources", 20),
        )
        self.bus.publish(
            "notebooklm.status.checked",
            "notebooklm",
            status.provider,
            payload={
                "source": auth.source,
                "autenticado": bool(auth.access_token),
                "notebook": status.default_notebook,
                "max_sources": status.max_sources,
            },
        )
        return status
